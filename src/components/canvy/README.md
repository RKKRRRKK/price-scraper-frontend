...
| `src/components/canvy/CanvyAiModal.vue` | The **AI assist** dialog: prompt picker (General / BPMN / C4 / Comment), section-scope toggle (+ a **context** sub-toggle: edit the selection but see the rest as read-only reference), instruction box, optional **steering** box, **Run with AI** button (**Review with AI** in comment mode) + **Max passes** slider (1–8; the AI self-stops early), an **"I want more"** resume button, a live **Thinking** trace (the model's per-pass note), status line (with token/cost), the model's **note** and a **flagged-issues** callout, a **debug folder** picker + **debug log** panel, and the manual copy/paste controls. Emits `run`/`more` (direct) and `build` (manual). |
...
| `src/lib/canvyPrompts.js` | **Prompt catalogue + builders** shared by both flows: `PROMPTS`/`promptMode`, per-mode **semantics** (General/BPMN/C4/Comment) injected into the op-DSL shell, `buildConstructivePrompt` (first pass) / `buildBuildPrompt` (continuation passes), and `buildManualPrompt` (copy/paste). |
...
| `src/lib/canvyAiLive.js` | **The direct-run orchestrator**: `callGemini()` (invokes the edge function, returns `{ text, usage }`) and `runLiveAi()` — the **reason-act-observe build loop**: each pass observes a screenshot, reasons (a `!` note threaded pass→pass), then acts (ops that add + fix). Self-stops on `done`, else runs up to `maxPasses`; `resume`+`priorNotes` continue a finished build. Comment mode is a single text-only call. DOM/store-free; the view injects `capture` and `commitData`. |
