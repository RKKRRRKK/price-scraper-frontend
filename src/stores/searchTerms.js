...
export const useSearchTerms = defineStore('searchTerms', {
  ...
  actions: {
    ...
    _mergePrice(priceRow) {
      ...
      if (
        term.lowestPrice == null ||
        (priceRow.current_lowest_price != null && priceRow.current_lowest_price < term.lowestPrice)
      ) {
    ...
    async fetchAll() {
      if (this.loaded || this.loading) {
        console.log(
          `[SearchTerms] Skipping fetchAll (Loaded: ${this.loaded}, Loading: ${this.loading})`,
        )
        return
      }
    ...
    async addTerm(termData) {
      // <-- Change to accept the whole object first
      ...
      console.log(
        '[SearchTerms] addTerm action DESTRUCTURED fileId:',
        fileId,
        'Type:',
        typeof fileId,
      )
      ...
      if (fileId === null || typeof fileId === 'undefined' || typeof fileId !== 'number') {
        console.error(
          '[SearchTerms] addTerm STOPPED post-destructure: fileId is missing or invalid!',
          fileId,
        )
      ...
      try {
        ...
        console.log(
          '[SearchTerms] Inserting into Supabase with payload:',
      ...
      try {
            ...
            (payload) => {
              ...
              if (eventType === 'INSERT' || eventType === 'UPDATE') {
                ...
                if (oldRecord && oldRecord.id) {
                  ...
                  if (this.terms.length < initialLength) {
                    console.log(
                      '[SearchTerms] Realtime <scrape_jobs> DELETE processed locally for ID:',
                      oldRecord.id,
                    )
                  }
                }
        ...
        this.realtimeChannelPrices = supabase
          ...
          .on(
            ...
            (payload) => {
              console.log(
                '[SearchTerms] Realtime <listings> event received, triggering price refresh:',
                payload.eventType,
              )
